...
import { FiPlus,FiMinus, FiBold} from "react-icons/fi";
...
const Cart = (props) => {
    ...
    return(
       ...
       <div className="cart" key={product.id}>
        ...
        <span className="inc" onClick={() => dispatch({type: 'INC', id:product.id})}><i className="fas fa-plus"><FiPlus/></i></span>
        <span className="productQuantity">{product.qty}</span>
        <span className="dec" onClick={() => dispatch({type: 'DEC', id: product.id})}><i className="fas fa-minus"><FiMinus/></i></span>
        <span className="productTotalPrice">${product.qty * product.price}.00</span>
        <button onClick={() => dispatch({type: 'DELETE_PRODUCT', id: product.id})} className="deleteCartPro"><i className="fas fa-trash-alt"><AiFillDelete/></i></button>
        
           </div>  
        )) : 'Yourr Cart is currently empty!'}
        <div><button style={{alignItems:"center",fontSize:20,background:'white',color:'green'}}>
        <Link to="/" >Add New Item</Link></button></div>
        </div>
